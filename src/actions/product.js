...
const getProducts = (search = '', page = '', refresh = false) => async (dispatch, getState) => {
    try {
        ...
        const contractInstance = new web3.eth.Contract(NFT_MARKETPLACE_ABI, CONTRACTS.MARKETPLACE);
        const nftInstance = new web3.eth.Contract(NFT_ABI, CONTRACTS.NFT);

        console.log('contractInstance: ', contractInstance)
        const [ids] = await makeBatchCall(contractInstance, [{ methodName: 'getNFTList', args: [] }])
        if (ids.length < 1) {
            return  
        }  
        ...
        for (let i = 0; i < ids.length; i++) {
            const [product] = await makeBatchCall(contractInstance, [{ methodName: 'getNFTProdByTokenID', args: [ids[i]] }])
            const [tokenURI] = await makeBatchCall(nftInstance, [{ methodName: 'tokenURI', args: [ids[i]] }])
            tokenProducts.push({
                ...product,
                uri: tokenURI,
            })
